  ...
  return (
    <>
      <div className="nav-pages px-5 space-y-2 pb-5 block">
        <span className={`uppercase text-textColor text-xs opacity-75`}>
          {data.pageName}
        </span>
        <div
        ...
        >
          <div className="flex items-center gap-2 md:gap-5">
            <span className="md:text-xl text-sm">{data.icon}</span>
            <div className="flex gap-2 items-center my-2">
              <span className="text-xs md:text-lg">{data.pageTitle}</span>
              <span
                className="text-warning rounded-lg md:block hidden"
          ...
          <ul className="navbar-bullet text-textColor pl-5 space-y-1 dropdown-anime">
            {data.subItems?.map((item, i) => (
              <li key={i} onClick={() => setActiveNavar(i)}>
                <Link href={item.url}>
                  <span
                    className={`md:pl-6 hover:text-white ${
